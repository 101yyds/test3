module div_unit (
    ...
    logic [ 3: 0] cycles_remaining_next        ;
    ...
    always_ff @(posedge clk) begin
        if (running)
            shifted_divisor <= {2'b0, shifted_divisor[31:2]};
        else
            shifted_divisor <= unsigned_divisor << {CLZ_delta[5-1:1], 1'b0};//Rounding down when CLZ_delta is odd
    end
    ...
    always_ff @(posedge clk) begin
        if (start)
            ...
            quotient <= {quotient[(32-3):0], new_quotient_bits};
    end
    ...
    always_ff @(posedge clk) begin
        if (start | (running & |new_quotient_bits)) begin  //enable: on div.start_delay for init and so long as we are in the running state and the quotient pair is not zero
            case ({~running, sub_1x_overflow})
                0: remainder <= sub_1x;
                1: remainder <= sub_2x;
                default: remainder <= unsigned_dividend; //Overloading the quotient zero case to fit the initial loading of the dividend in
            endcase
        end
    end
    ...
    always_ff @(posedge clk) begin
        cycles_remaining <= running ? cycles_remaining_next : CLZ_delta[5-1:1];
    end
